...
*****************************************************************
*/
void IncPID_Calculation(float dT,float Expect,float Freedback,_PID_Arg_ * PID_Arg,_PID_Val_ * PID_Val, float Error_Lim,float Integral_Lim)
{
	PID_Val->Error = Expect - Freedback;//误差 = 期望值-反馈值
	PID_Val->Error = LIMIT(PID_Val->Error, -Error_Lim, Error_Lim);
