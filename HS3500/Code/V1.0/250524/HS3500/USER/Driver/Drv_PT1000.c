...
/**********局部变量**********/
uint16_t ADC_Val[AD_LEN];//adc的值 0:台面温度ad值 1：外部探头 2：TL431的ad值 3：单片机内部的电压AD（参考是1.2f）
...
{
    static float T;
    float TL431_AD;//TL431基准2.5V电压的ADC值
    float Filter_PT1;//滤波后的PT1000的ADC值台面
	float Filter_PT2;//滤波后的PT1000的ADC值外部
    
	T += dT;
    
//    Vref_AD = Filter_ADC(3);//读取并滤波单片机内部电压的ADC值
    TL431_AD = Filter_ADC(2);//读取并滤波TL431电压的ADC值
//    TL431_V = (TL431_AD / Vref_AD) * Vref;//计算TL431的电压值
    TL431_V = 2.5f;//计算TL431的电压值
	
//    Filter_PT1 = Filter_ADC(1);//读取并滤波PT1000的ADC值外部
//    PT_VALUE_1_TEMP = Filter_ADC_EMA(Filter_PT1,1) *1.0172f ;//计算阻值外部
//    
//    Filter_PT2 = Filter_ADC(0);//读取并滤波PT1000的ADC值台面
//    PT_VALUE_2_TEMP = Filter_ADC_EMA(Filter_PT2,0)  *1.0172f;//计算阻值台面
    
    Filter_PT1 = ADC_Val[1];//读取并滤波PT1000的ADC值外部
    PT_VALUE_1_TEMP = Filter_ADC_EMA(Filter_PT1,1) * (1000 /Param.Correct);//计算阻值外部
    
    Filter_PT2 = ADC_Val[0];//读取并滤波PT1000的ADC值台面
    PT_VALUE_2_TEMP = Filter_ADC_EMA(Filter_PT2,0) * (1000 /Param.Correct);//计算阻值台面
