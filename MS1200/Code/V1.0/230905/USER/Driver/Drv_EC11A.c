	...
	if(GPIO_Pin == EC11A[0].EXTI_Pin)//A上升沿触发外部中断
	{
		...
		while(EC11A[0].TIM_Cnt <= 2)//定时器一个周期1ms，计时2ms内看看A有没有电跳变
		{
			...
			{ 
				...
				if(sys.Run_Status)
					EC11A[0].EC11A_Knob = 4.0f;//在旋转旋钮时
				else
					EC11A[0].EC11A_Knob = 2.0f;//在旋转旋钮时
				if(HAL_GPIO_ReadPin(EC11A[0].EC11A_GPIO,EC11A[0].EC11A_Pin) == 1)//减
				{
				...
				else if(HAL_GPIO_ReadPin(EC11A[0].EC11A_GPIO,EC11A[0].EC11A_Pin) == 0)//加
				{
					if(EC11A[0].EC11A_Speed < EC11A[0].EC11A_Fast)//如果慢慢旋转
						Speed.Set += 10;
					else
