...
uint16_t ADC_Val[AD_LEN];//adc的值 0:防干烧。 1：ad值;
...
void Read_Temp(float dT)
{
    ...
    if(T >= 1.0f)//1S
    {
		...
		Temp.Rel_Speed = safe_div((float)Temp.Rel - (float)RelTemp_Old,2.0f,0);//计算实际温度速度
		Temp.Alarm_Speed = safe_div((float)Temp.Alarm - (float)AlarmTemp_Old,2.0f,0);//计算报警温度速度
		T = 0;
    }
}
