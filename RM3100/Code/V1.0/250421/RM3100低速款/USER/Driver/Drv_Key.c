	...
	if(Key_Flag1 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt1 < 1.5)//小于1.5S是单击
			{
				...
				{
					Speed_Val.SumError = 0x01BC;//启动电机系数
					SetOK_Flag = 1;//设定数值标志
	...
	if(Key_Flag2 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt2 < 1.5)//小于1.5S是单击
			{
				...
				{
					Set_Speed +=1;//速度++
					if(Set_Speed > 30)//速度大于80时
						Set_Speed = 30;//速度最大为80就上不去了
				...
				else if(sys.SetMode_Option == 2)//在设置时间时
				{
					...
					if(Set_Time>86340)//时间大于23小时59分
						Set_Time = 86340;//时间最大为23小时59分
					Key_Status = 1;//按键一秒，表示按下时有1s的等待，不闪烁
					Twinkle_Time = 6;//一共闪烁6S
				}
				else
	...
	if(Key_Flag3 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt3 < 1.5)/*单击*///小于1.5S是单击
			{
				...
				{
					Set_Speed -=1;//速度--
					if(Set_Speed < 5)//速度小于20
						Set_Speed = 5;//速度最小为20就下不去了
					Key_Status = 1;//按键一秒，表示按下时有1s的等待，不闪烁
					Twinkle_Time = 6;//一共闪烁6S
				}
				else if(sys.SetMode_Option == 2)//在设置时间时
