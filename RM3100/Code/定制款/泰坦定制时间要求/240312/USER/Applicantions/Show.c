...
/**********局部变量声明******/
uint8_t TimeM_ShowFlag,TimeH_ShowFlag,Speed_ShowFlag;//时间、速度显示的标志位 0:常亮 1：熄灭
    ...
    if(Twinkle_Time && Key_Status==0)//闪烁和没有操作旋钮时
    {
		...
		if(T >= 0.5f)//大于0.5S后
		{
			...
			if(sys.SetMode_Option == 1)//设置速度
			{
				...
				TimeM_ShowFlag = 0;//时间常亮
				TimeH_ShowFlag = 0;//时间常亮
			}
			else if(sys.SetMode_Option == 2)//设置时间
			{
				Speed_ShowFlag = 0;//速度常亮
				TimeM_ShowFlag = 0;//时间常亮
				TimeH_ShowFlag = ~TimeH_ShowFlag;//时间闪烁
			}
			else if(sys.SetMode_Option == 3)//设置时间
			{
				Speed_ShowFlag = 0;//速度常亮
				TimeM_ShowFlag = ~TimeM_ShowFlag;//时间闪烁
				TimeH_ShowFlag = 0;//时间常亮
			}
