	...
	if(Key_Flag3 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt3 < 1.5)/*单击*///小于1.5S是单击
			{
				...
				if(sys.SetMode_Option == 2)//在设置时间小时
				{
					Set_Time -= 3600;//每次时间减一小时
					if(Set_Time < 60)//时间小于一分钟时
						Set_Time = 0;//设置时间为0
				}
				if(sys.SetMode_Option == 3)//在设置时间分钟
				{
	...
	if(Key_Flag4 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt4 < 1.5)//小于1.5S是单击
			{
				...
				{
					if(sys.SetMode_Option > 3)//设置选项大于2
					{
						sys.SetMode_Option = 0;//设置选项清零
					}
				}
