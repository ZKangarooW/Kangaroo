...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance == TIM2)
    {
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
        {
            capture1[4] = __HAL_TIM_GET_COMPARE(&htim2, TIM_CHANNEL_1);//获取Tim3通道1的输入捕获
            CaptureChanneCallback(4);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)
        {
            capture1[5] = __HAL_TIM_GET_COMPARE(&htim2, TIM_CHANNEL_2);//获取Tim3通道2的输入捕获
            CaptureChanneCallback(5);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_3)
        {
            capture1[6] = __HAL_TIM_GET_COMPARE(&htim2, TIM_CHANNEL_3);//获取Tim3通道3的输入捕获
            CaptureChanneCallback(6);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_4)
        {
            capture1[7] = __HAL_TIM_GET_COMPARE(&htim2, TIM_CHANNEL_4);//获取Tim3通道4的输入捕获
    ...
    if (htim->Instance == TIM4)
    {
        ...
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)
        {
            capture1[9] = __HAL_TIM_GET_COMPARE(&htim4, TIM_CHANNEL_2);//获取Tim3通道2的输入捕获
            CaptureChanneCallback(9);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_3)
        {
            capture1[10] = __HAL_TIM_GET_COMPARE(&htim4, TIM_CHANNEL_3);//获取Tim3通道3的输入捕获
    ...
    if (htim->Instance == TIM8)
    {
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
        {
            capture1[11] = __HAL_TIM_GET_COMPARE(&htim8, TIM_CHANNEL_1);//获取Tim3通道1的输入捕获
            CaptureChanneCallback(11);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)
        {
            capture1[12] = __HAL_TIM_GET_COMPARE(&htim8, TIM_CHANNEL_2);//获取Tim3通道2的输入捕获
            CaptureChanneCallback(12);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_3)
        {
            capture1[13] = __HAL_TIM_GET_COMPARE(&htim8, TIM_CHANNEL_3);//获取Tim3通道3的输入捕获
            CaptureChanneCallback(13);
        }
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_4)
        {
            capture1[14] = __HAL_TIM_GET_COMPARE(&htim8, TIM_CHANNEL_4);//获取Tim3通道4的输入捕获
