...
void Read_Temp(float dT)
{
	...
	{
		PT_VALUE_1_TEMP[i] = Filter_ADC_EMA(ADC_Val[i * 2],(i * 2 + 1)) * (1000 /Param.Correct[i]);//计算阻值外部
		PT_VALUE_2_TEMP[i] = Filter_ADC_EMA(ADC_Val[i * 2 + 1],(i * 2)) * (1000 /Param.Correct[i]);//计算阻值台面
	}
