...
/**********局部变量**********/
uint16_t ADC_Val[AD_LEN];//adc的值 0:台面温度ad值 1：外部探头
...
{   
    static float EMA_value[AD_LEN];//EMA滤波后的值，因为需要更新所以用静态
    float V_out;//电压值
    float R_value;//电阻值
...
//    V_out = ((float)ADC_value / (float)Vref_AD) * Vref;
	...
	V_out = ((float)ADC_value / 4096) * 3.3f;
...
{
    static float T;
    float Filter_PT1[Motor_Num];//滤波后的PT1000的ADC值台面
	float Filter_PT2[Motor_Num];//滤波后的PT1000的ADC值外部

	T += dT;
    TL431_V = 2.5f;//计算TL431的电压值
	
	for(uint8_t i = 0;i<Motor_Num;i++)
	{
		Filter_PT1[i] = ADC_Val[i * 2];      // 外部 PT1000 的 ADC 值
		Filter_PT2[i] = ADC_Val[i * 2 + 1];  // 台面 PT1000 的 ADC 值
		PT_VALUE_1_TEMP[i] = Filter_ADC_EMA(Filter_PT1[i],(i * 2 + 1)) * (1000 /Param.Correct[i]);//计算阻值外部
		PT_VALUE_2_TEMP[i] = Filter_ADC_EMA(Filter_PT2[i],(i * 2)) * (1000 /Param.Correct[i]);//计算阻值台面
	...
	{  
		for(uint8_t i = 0;i<Motor_Num;i++)
        {		
			Work_Num[i].Temp.Outside = Wolfram_Inverse(PT_VALUE_1_TEMP[i]);//外部温度
			Work_Num[i].Temp.Mesa = Wolfram_Inverse(PT_VALUE_2_TEMP[i]);//台面温度
