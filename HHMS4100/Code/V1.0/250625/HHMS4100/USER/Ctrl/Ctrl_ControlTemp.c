...
{
    if (Work_Num[i].Temp.Outside < Ctrl_temp - 180)//达到目标温度-15℃前全速加热
    {
        Mesa_Ctrl(dT, i, 3100);
    }
    else if (Work_Num[i].Temp.Outside < Ctrl_temp - 100)//达到目标温度-10℃前全速加热
    {
        Mesa_Ctrl(dT, i, Ctrl_temp*2.5f);
    }
    else//PID控制
    {//设定加热周期为1000ms，1000分之PID_Para.pid_result毫秒开加热器
        if(Ctrl_temp>700)
        ...
        {
            AltPID_Calculation(dT, Ctrl_temp, Work_Num[i].Temp.Outside, &Temp_Out_Arg, &Temp_Out_Val[i], 2500, Ctrl_temp*1.2);
            if(Temp_Out_Val[i].Out < 0)
                Temp_Out_Val[i].Out = 0;
