	...
	if(EC11A[0].Key_Flag == 1)//按键被按下
	{
		...
		{   
			if(EC11A[0].Key_Cnt > 0.1 && EC11A[0].Key_Cnt < 1.5)//小于1.5S是单击
			{
				...
				{
					if(sys.Run_Status != 1 && Speed.Set)//系统没启动的话
					{
						sys.Run_Status = 1;//启动系统
						Speed.ADDMode = 0;//进入判断速度显示处理
						Speed_Val.Integral = 50;//电器起步
...
{
	Speed.Set = Param.P_Param[PMode.Option][0];//将Flash中的速度赋值
	Time.Set = Param.P_Param[PMode.Option][1];//将Flash中的温度赋值
