...
void Key_Scan(float dT)
{
	...
	if(Key_Flag2 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt2 < 1.4)//小于1.5S是单击
			{
				...
				{
					Temp.Set_Temp += 10;//温度加1度
					if(Temp.Set_Temp > Temp_MAX)//假如温度大于Temp_MAX度时
						Temp.Set_Temp = Temp_MAX;//温度等于Temp_MAX度
				}
				else if(sys.SetMode_Option == 2)//设置速度
				{
					Speed.Set_Speed += 10;//转速加10转
					if(Speed.Set_Speed == 10)//从零转开始最低为50转，判断是10后
						Speed.Set_Speed = 50;//设定转速为50开始
					if(Speed.Set_Speed > Speed_MAX)//假如转速大于Speed_MAX
						Speed.Set_Speed = Speed_MAX;//转速等于Speed_MAX
				}
				else if(sys.SetMode_Option == 3)//设置时间
				{
					Time.Set_Time += 60;//时间加1分钟
					if(Time.Set_Time > Time_MAX)//假如时间大于Time_MAX时
						Time.Set_Time = Time_MAX;//时间等于Time_MAX
				}
		...
		if(Key_Cnt2 > 1.9 && Key_Cnt2 < 2.1)//按键时间大于1.9S小于2.1S表示长按
		{
			if(sys.SetMode_Option == 1)//设置温度
			{
				Temp.Set_Temp += 100;//温度加10度
				if(Temp.Set_Temp > Temp_MAX)//假如温度大于Temp_MAX度时
					Temp.Set_Temp = Temp_MAX;//温度等于Temp_MAX度
			}
			else if(sys.SetMode_Option == 2)//设置速度
			{
				Speed.Set_Speed += 100;//转速加100转
				if(Speed.Set_Speed > Speed_MAX)//假如转速大于Speed_MAX
					Speed.Set_Speed = Speed_MAX;//转速等于Speed_MAX
			}
			else if(sys.SetMode_Option == 3)//设置时间
			{
				Time.Set_Time += 600;//时间加10分钟
				if(Time.Set_Time > Time_MAX)//假如时间大于Time_MAX时
					Time.Set_Time = Time_MAX;//时间等于Time_MAX
			}
			Key_Status = 2;//设置时2S不闪烁
	...
	if(Key_Flag3 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt3 < 1.4)/*单击*///小于1.5S是单击
			{
				if(sys.SetMode_Option == 1)//设置温度
				{
					Temp.Set_Temp -= 10;//温度减1度
					if(Temp.Set_Temp < 0)//假如温度小于0度时
						Temp.Set_Temp = 0;//温度等于0度
				}
				else if(sys.SetMode_Option == 2)//设置速度
				{
					Speed.Set_Speed -= 10;//转速减10转
					if(Speed.Set_Speed < 50)//假如转速小于50时
						Speed.Set_Speed = 0;//转速等于0
				}
				else if(sys.SetMode_Option == 3)//设置时间
				{
					Time.Set_Time -= 60;//时间减1分钟
					if(Time.Set_Time < 0)//假如时间小于0时
						Time.Set_Time = 0;//时间等于0
				}
				Key_Status = 2;//设置时2S不闪烁
				Twinkle_Time = 6;//闪烁时间6S
			}
		...
		if(Key_Cnt3 > 1.9 && Key_Cnt3 < 2.1)//按键时间大于1.9S小于2.1S表示长按
		{
			if(sys.SetMode_Option == 1)//设置温度
			{
				Temp.Set_Temp -= 100;//温度减10度
				if(Temp.Set_Temp < 0)//假如温度小于0度时
					Temp.Set_Temp = 0;//温度等于0度
			}
			else if(sys.SetMode_Option == 2)//设置速度
			{
				Speed.Set_Speed -= 100;//转速减100转
				if(Speed.Set_Speed < 50)//假如转速小于50时
					Speed.Set_Speed = 0;//转速等于0
			}
			else if(sys.SetMode_Option == 3)//设置时间
			{
				Time.Set_Time -= 600;//时间减10分钟
				if(Time.Set_Time < 0)//假如时间小于0时
					Time.Set_Time = 0;//时间等于0
			}
			Key_Status = 2;//设置时2S不闪烁
	...
	if(Key_Flag4 == 1)//按键被按下
	{
		...
		{   
			...
			{
				if(sys.Run_Status == 0 && (Speed.Set_Speed || Temp.Set_Temp))//系统没启动的话
				{
					sys.Run_Status = 1;//启动系统
					Speed_Val.SumError = 0x12E58;//给pid的积分和来起步电机
					sys.SetMode_Option = 0;
				}
				else//系统启动的话
				{
