			...
			{ 
				...
				{	
					if(EC11A[0].EC11A_Speed < EC11A[0].EC11A_Fast)//如果慢慢旋转
						Set_Time += 60;//设定时间加60S
					else
						Set_Time += 600;//设定时间加600S
					Set_Time = (Set_Time > Time_MAX) ? Time_MAX : Set_Time;//时间最多设定24小时
					Time_State  = (Set_Time < 60) ? 0 : 1;//判断是否设置了时间
					Twinkle_Time2 = 2.0f;//时间闪烁2S
					break;
				}
				else if(HAL_GPIO_ReadPin(EC11A[0].EC11A_GPIO,EC11A[0].EC11A_Pin) == 1)//减
				{
					if(EC11A[0].EC11A_Speed < EC11A[0].EC11A_Fast)//如果慢慢旋转
						Set_Time -= 60;//设定时间减60S
					else
						Set_Time -= 600;//设定时间减600S
					Set_Time = (Set_Time < Time_MIN) ? 0 : Set_Time;//时间小于1分钟不设定，设定时间为0（常动）
					Twinkle_Time2 = 2.0f;//时间闪烁2S
					Time_State  = (Set_Time < 60) ? 0 : 1;//判断是否设置了时间
					break;
				}
			...
			{ 
				...
				if(HAL_GPIO_ReadPin(EC11A[1].EC11A_GPIO,EC11A[1].EC11A_Pin) == 0)//加
				{
                    if(EC11A[1].EC11A_Speed < EC11A[1].EC11A_Fast)//如果慢慢旋转
						...
						Set_Speed += 50;//转速加50
					Set_Speed = (Set_Speed > Speed_MAX) ? Speed_MAX : Set_Speed;//速度不超过最快值
					Twinkle_Time1 = 2.0f;//速度闪烁2S
					break;
				}
				else if(HAL_GPIO_ReadPin(EC11A[1].EC11A_GPIO,EC11A[1].EC11A_Pin) == 1)//减
				{
                    if(EC11A[1].EC11A_Speed < EC11A[1].EC11A_Fast)//如果慢慢旋转
						...
						Set_Speed -= 50;//转速减50
					Set_Speed = (Set_Speed < Speed_MIN) ? Speed_MIN : Set_Speed;//速度不小于最慢值
					Twinkle_Time1 = 2.0f;//速度闪烁2S
					break;
				}
				break;
