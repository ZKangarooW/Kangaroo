...
void Deal_Speed(void)
{
    ...
    if(sys.Run_Status == 1)//启动的情况下
    {
        ...
        {
            if(Speed.Ctrl > Speed.Rel)//控制速度大于实际速度
            {
                Speed.New =0;//现在的速度清零
                Speed.Last = 0;//之前的速度清零
                Speed.ADDMode = 1;//进入加速模式下
            }
            else if(Speed.Ctrl <= Speed.Rel)//控制速度小于显示速度
            {
                Speed.New=0;//现在的速度清零
                Speed.Last = Speed.Rel;//之前的速度等于当前显示速度
                Speed.ADDMode = 2;//进入减速模式下
            }
        }
        if(Speed.ADDMode==1)//在进入加速模式下
        { 
			if(Speed.Rel >= Speed.Ctrl)//实际速度大于等于控制速度
			{
        ...
        else if(Speed.ADDMode == 2)//速度下降模式下
        {
            if(sys.Motor_Stop == 0)
            {
                if(Speed.Rel <= Speed.Ctrl)//实际速度小于等于控制速度
                {
