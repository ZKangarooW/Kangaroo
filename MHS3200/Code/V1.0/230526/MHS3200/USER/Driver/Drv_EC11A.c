	...
	if(EC11A[1].Key_Flag == 1)//按键被按下
	{
		...
		{   
			if(EC11A[1].Key_Cnt > 0.1 && EC11A[1].Key_Cnt < 1.5)//小于1.5S是单击
			{
                ...
                #if(Temp_TYPE == 0)//设置成普通款时
                    if(SetMode_Option == 2)
                        SetMode_Option = 3;//直接跳过温度
                    if(SetMode_Option == 4)//设置模式只有2种，等于4时
                    {
                        SetMode_Option = 0;//清零
                    }
                #elif(Temp_TYPE == 1)//设置成加热款时
                    if(SetMode_Option == 4)//设置模式只有3种，等于4时
                    {
                        SetMode_Option = 0;//清零
                    }
                #endif
