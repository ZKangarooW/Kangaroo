...
uint8_t Lock_ShowFlag,TimeIcn_ShowFlag,P_ShowFlag;//开盖图标和时间图标闪烁和P模式闪烁
...
/*
*****************************************************************
 * 函数原型：static void Start_CircleRun(float dT)
 * 功    能：外框跑圈动画
 * 调    用：内部调用
*****************************************************************
*/
static void Start_CircleRun(float dT)
{
	static float T;
	if(sys.Run_Status)
	{
...
static void Check_ShowFlag(float dT)
{
    ...
    if(Twinkle_Time && Key_Status==0)//闪烁和没有操作按键时
    {
		...
		if(T >= 0.5f)
		{
			...
			else if(sys.SetMode_Option == 1)//设置P值
			{
				Speed_ShowFlag = 0;//速度常亮
				Time_ShowFlag = 0;//时间常亮
...
/*
*****************************************************************
 * 函数原型：void Display_RunMode(int16_t temp,uint8_t P)
 * 功    能：显示安全温度和记忆模式
 * 输    入: temp 安全温度  P 记忆模式
 * 参    数：uint16_t temp,uint8_t P
*****************************************************************
*/
void Display_RunMode(int16_t temp,uint8_t P)
{
	...
	if(Temp_ShowFlag == 0)
	{
		...
		else if (temp >= 0) // 大于99时
        {
            temp1 = 11;
            temp2 = 11;
            temp3 = ABS(temp) / 10 % 10;
            temp4 = ABS(temp) % 10;
        }
        else if (temp > -100) // 大于99时
        {
            temp1 = 11;
            temp2 = 10;
            temp3 = ABS(temp) / 10 % 10;
        ...
        else if (temp > -1000) // 大于99时
        {
            temp1 = 10;
            temp2 = ABS(temp) / 100 % 10;
            temp3 = ABS(temp) / 10 % 10;
            temp4 = ABS(temp) % 10;
        }
		switch(temp1)
		{
			...
			case 9:seg29&=0xF0;seg28&=0xF8;seg29|=0x0B;seg28|=0x07;//数字9
				break;
			case 10:seg29&=0xF0;seg28&=0xF8;seg29|=0x00;seg28|=0x02;//显示“-”
				break;
			case 11:seg29&=0xF0;seg28&=0xF8;seg29|=0x00;seg28|=0x00;//不显示
				break;
			default:
				break;
		}
		switch(temp2)
		{
			...
			case 9:seg27&=0xF0;seg26&=0xF8;seg27|=0x0B;seg26|=0x07;//数字9
				break;
			case 10:seg27&=0xF0;seg26&=0xF8;seg27|=0x00;seg26|=0x02;//数字9
				break;
			case 11:seg27&=0xF0;seg26&=0xF8;seg27|=0x00;seg26|=0x00;//不显示
				break;
			default:
				break;
		}
		switch(temp3)
		{
			...
			case 9:seg25&=0xF0;seg24&=0xF8;seg25|=0x0B;seg24|=0x07;//数字9
				break;
			case 10:seg25&=0xF0;seg24&=0xF8;seg25|=0x00;seg24|=0x02;//数字9
				break;
			case 11:seg25&=0xF0;seg24&=0xF8;seg25|=0x00;seg24|=0x00;//数字0
				break;
			default:
				break;
		}
...
void Deal_Speed(void)
{
    ...
    {
        if(Rel_Speed >= 8000)
            Rel_Val = 12000-(10000 - Rel_Speed)*2;
        else
            Rel_Val = Rel_Speed;
    }
    else
    ...
    if(sys.Run_Status == 1)//启动的情况下
    {
        ...
        { 
            if (Rel_Val > Display_RelSpeed) // 当前速度大于显示速度
            {
                if (Display_RelSpeed < Rel_Val)
                    Display_RelSpeed += 1; // 显示当前速度
            }
            else // 当前速度小于上一次速度
            {
                Display_RelSpeed = Display_RelSpeed; // 显示上一次速度，不让速度小于当前速度。呈现攀升速度的现象
            }
        ...
        { 
            if (Rel_Val < Display_RelSpeed) // 当前速度小于上一次速度
            {
                if (Display_RelSpeed > Rel_Val)
                    Display_RelSpeed -= 1; // 显示当前速度
            }
            ...
            {
                Display_RelSpeed = Display_RelSpeed; // 显示上一次速度，不让速度大于当前速度。呈现下降速度的现象
            }
        }
	...
	{
		if (Display_RelSpeed && Display_RelSpeed > Rel_Val)
			Display_RelSpeed -= 1; // 显示当前速度
	}
}
...
//	if(sys.SetMode_Option == 4)
//	{
//		Display_SafeTemp = Safe_Set_Temp;
//	}
//	else
//	{
