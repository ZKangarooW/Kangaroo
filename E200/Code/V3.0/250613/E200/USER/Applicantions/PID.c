...
void IncPID_Calculation(float dT,float Expect,float Freedback,_PID_Arg_ * PID_Arg,_PID_Val_ * PID_Val,float Integral_Lim)
{
	...
	PID_Val->Proportion   = PID_Arg->Kp * PID_Val->Error;//比例 = 比例系数*（当前误差-上一次误差）
	...
	PID_Val->Integral     += PID_Val->Error* dT;//积分 = 积分系数*误差*周期
	PID_Val->Integral     = LIMIT(PID_Val->Integral,-Integral_Lim,Integral_Lim);//积分限幅
	PID_Val->Differential = PID_Arg->Kd * (PID_Val->Error - PID_Val->Error_Last) * safe_div(1.0f,dT,0);//微分 = 微分系数 * （当前误差-2*上一次误差+上上次误差）*频率
	
	PID_Val->Out = PID_Val->Proportion + PID_Arg->Ki *PID_Val->Integral + PID_Val->Differential;//PID输出
