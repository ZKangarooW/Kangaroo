...
void Deal_Temp(float dT)
{
    ...
    else if(ABS(Heat_Temp.Rel_Temp - Heat_Temp.Ctrl_Temp)>=14)
    {
        if(Heat_Temp.ADDMode)
            T += dT;
        if(T >= 2.0f)
	...
	if(sys.Calibration)
	{
		if(sys.Calibration_Step == 0)
		{
			...
			else if(260 - Cool_Temp.Rel_Temp > 14 || 260 - Cool_Temp.Rel_Temp < -14 )
			{
				if(Cool_Temp.ADDMode)
					T += dT;
				if(T >= 2.0f)
		...
		else
		{
			...
			else if(400 - Cool_Temp.Rel_Temp > 14 || 400 - Cool_Temp.Rel_Temp < -14 )
			{
				if(Cool_Temp.ADDMode)
					T += dT;
				if(T >= 2.0f)
	...
	else
	{
        ...
        else if(Cool_Temp.Ctrl_Temp - Cool_Temp.Rel_Temp > 14 || Cool_Temp.Ctrl_Temp - Cool_Temp.Rel_Temp < -14 )
        {
            if(Cool_Temp.ADDMode)
                T1 += dT;
            if(T1 >= 2.0f)
...
{	
	...
	else//倒计时
	{
		if(sys.SetMode_Option == 3)
			...
			Time.Display_Time = Time.Rel_Time;
		if(sys.Calibration)
			flag = 1;
		else
			flag = 0;
