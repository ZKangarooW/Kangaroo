...
void CoolTemp_Mode(float dT,float pwm)
{
	static float T;
//	static 
	
	if(pwm >= 40 && (Cool_Temp.Ctrl_Temp > Cool_Temp.Rel_Temp))
	{
		if(pwm > 50)
			WIND_OFF;
		mode = 0;//加热
	}
	else if(pwm < 40 && (Cool_Temp.Ctrl_Temp < Cool_Temp.Rel_Temp))
	{
		if(pwm < -60)
			WIND_ON;
		mode = 1;//制冷
	...
	{
		if(Cool_Temp.Ctrl_Temp > Cool_Temp.Rel_Temp)
		{
			HEAT = (int)(ABS(pwm*10));
            COLD = 0;
		}			
		else if(Cool_Temp.Ctrl_Temp < Cool_Temp.Rel_Temp)
		{
            HEAT = 0;
			COLD = (int)(ABS(pwm*10));
		}
...
{ 
	...
	else if(sys.Calibration == 1)
	{
		...
		if(sys.Calibration_Step == 0)
		{
			// 自动调参阶段
            if (!tuning_complete) {
                tuning_complete = AutoTunePID(dT,400, &Ku, &Tu);
                if(tuning_complete)
                {
                    HeatTemp_Arg.Kp = 0.6 * Ku;
                    HeatTemp_Arg.Ki = 2 * HeatTemp_Arg.Kp_i / Tu;
            ...
            setPWM(400,HeatTemp_Val.Out);
		}
		else
		{
			// 自动调参阶段
            if (!tuning_complete) {
                tuning_complete = AutoTunePID(dT, 600,&Ku, &Tu);
                if(tuning_complete)
                {
                    HeatTemp_Arg.Kp = 0.6 * Ku;
                    HeatTemp_Arg.Ki = 2 * HeatTemp_Arg.Kp_i / Tu;
