...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	if(CAPTURE_Status==0)
	{
		...
		{	  			
			...
			temp*=100;//一个周期溢出的时间（us）计算=定时器周期（1/（48000000/12）*200）=0.0001S=100us
			temp+=TIM1CH1_CAPTURE_VAL;//一个周期所需的us数（用溢出的时间加上没有溢出的时间）得出一个周期用了多少us
			temp=60000000/temp/16/30;//一分钟有60000000us，一分钟内有多少个脉冲（周期）。9是一圈有几个脉冲得出一分钟转了多少圈。56是减速比
			Rel_Speed=temp;
			P_Status=0;//捕获周期计数关闭
