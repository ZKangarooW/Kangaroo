...
void Check_Infrared(float dT)
{
    ...
    else if(Infrared[0].Someone==1)
    {
        T += dT;
        if(T >= 2.0f)
        {
            Infrared[0].Someone = 2;
            T=0;
        }
    }
    else if(Infrared[0].Someone==2)
    {
        T += dT;
        if(T >= 0.2f)
        {
            if(Infrared[0].Distance > 1500&&Infrared[0].Distance<3000)
            {
                Infrared[0].Someone = 3;
                T = 0;
            }
            else
            {
                Infrared[0].Someone = 0;
                if(Infrared[1].Someone != 3)
                    Beep_Flash = 2;
                T = 0;
            }
        }
    }
    else if(Infrared[0].Someone==3)
    {
        T += dT;
        if(Infrared[0].Distance > 1500&&Infrared[0].Distance<3000)
        {
    ...
    else if(Infrared[1].Someone==1)
    {
        T1 += dT;
        if(T1 >= 2.0f)
        {
            Infrared[1].Someone = 2;
            T1=0;
        }
    }
    else if(Infrared[1].Someone==2)
    {
        T1 += dT;
        if(T1 >= 0.2f)
        {
            if(Infrared[1].Distance > 1500&&Infrared[1].Distance<3000)
            {
                Infrared[1].Someone = 3;
                T1 = 0;
            }
            else
            {
                Infrared[1].Someone = 0;
                if(Infrared[0].Someone != 3)
                    Beep_Flash = 2;
                T1 = 0;
            }
        }
    }
    else if(Infrared[1].Someone==3)
    {
        T1 += dT;
        if(Infrared[1].Distance > 1500&&Infrared[1].Distance<3000)
        {
...
void Check_Infrared_Rec(float dT)
{
    ...
    if (Infrared[0].SSI_CNT >= 0.05) // 0.5s发现没出发输入捕获
    {
        Infrared[0].Distance = 10000;// 将红外置为10000
        Infrared[0].SSI_CNT = 0; // 计数清零
    }
    if (Infrared[1].SSI_CNT >= 0.05) // 0.5s发现没出发输入捕获
    {
        Infrared[1].Distance = 10000;// 将红外置为10000
        Infrared[1].SSI_CNT = 0; // 计数清零
    }
