...
/*
*****************************************************************
 * 函数原型：void Infrared_SSI(float dT)
 * 功    能：红外信号强度
 * 输    入：dT：运行周期
 * 参    数：float dT
*****************************************************************
...
void Infrared_SSI(float dT)
{
    ...
    if(Infrared[0].SSI)
    {
        sys.Infrared_SSI1 = Infrared[0].SSI;//信号赋值
    }
    else
    {
        sys.Infrared_SSI1 = 0;//信号归零
    }
    ...
    if(Infrared[1].SSI)
    {
        sys.Infrared_SSI2 = Infrared[1].SSI;//信号赋值
    }
    else
    {
        sys.Infrared_SSI2 = 0;//信号归零
    }
}
