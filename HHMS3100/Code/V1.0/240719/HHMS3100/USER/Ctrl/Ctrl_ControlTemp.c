...
void Temp_PID(void)
{
    ...
    {
        Temp_Arg[i].Kp = 1000*0.001f;
        Temp_Arg[i].Ki = 18*0.0001f;
...
static void Mesa_Ctrl(float dT, uint8_t i,int32_t Ctrl_temp)
{
    ...
    if ((Ctrl_temp > 1000)&&(Work_Num[i].Temp.Rel < Ctrl_temp - 150))//达到目标温度-0.5℃前全速加热
    {
        Heat[i] = 399;
    }
    else if ((Ctrl_temp <= 1000)&&Work_Num[i].Temp.Rel < Ctrl_temp - 100)//达到目标温度-0.5℃前全速加热
    {
        T[i] += dT;
        if(T[i] <3.0f)
            Heat[i] = 399;
    ...
    else if ((Ctrl_temp > 1000)&&Work_Num[i].Temp.Rel < Ctrl_temp)//达到目标温度-0.5℃前全速加热
    {
        Heat[i] = 399;
    }
    ...
    {
        Heat[i] = 0;
        Temp_Val[i].Integral = 0;
    }   
    else    //PID控制
    {   //设定加热周期为1000ms，1000分之PID_Para.pid_result毫秒开加热器
        ctrl_val[i] = Get_ADCVal(Ctrl_temp);//查表读ADC值
        
        adc_val[i] = PT_VALUE_2_TEMP[i];//实际的adc值
        
        if(Ctrl_temp > 2300 && Work_Num[i].Temp.Mesa > 2000)//如果控制温度大于230度，台面温度超过200度
            Out_Enable[i] = 1;//打开积分计算
        else if((ctrl_val[i] - adc_val[i] > -200 && ctrl_val[i] - adc_val[i] < 200))
            Out_Enable[i] = 1;//打开积分计算
        else
            Out_Enable[i] = 0;//关闭积分计算
        
        AltPID_Calculation(dT, ctrl_val[i], adc_val[i], &Temp_Arg[i], &Temp_Val[i], 100, Out_Enable[i] * 100);
        if(Temp_Val[i].Out < 0)
            Temp_Val[i].Out = 0;
