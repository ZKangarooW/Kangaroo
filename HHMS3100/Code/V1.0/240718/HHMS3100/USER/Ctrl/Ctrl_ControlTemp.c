...
static void Mesa_Ctrl(float dT, uint8_t i,int32_t Ctrl_temp)
{
    ...
    T[i] += dT;
    if (T[i] <= (float)(Temp_Val[i].Out/10))
    {
        Heat[i] =300;
    }
    else
...
{ 
    ...
    for(uint8_t i = 0;i<3;i++)
    {
        if(Work_Num[i].Run_Status && PT_VALUE_1_TEMP[i] >= 2200)//启动系统控制台面
        {
            ...
            {
                if (Work_Num[i].Temp.Rel < Work_Num[i].Temp.Ctrl - 100)//达到目标温度-0.5℃前全速加热
                {
                    T[i] += dT;
                    if(T[i] <2.0f)
                        Heat[i] = 350;
                    else if(T[i] >= 2.0f && T[i] < 5.0f)
                        Heat[i] = 0;
                    else
                         T[i] = 0;
                }
                else if (Work_Num[i].Temp.Rel >= Work_Num[i].Temp.Ctrl + 10) //超出设置值1℃时全关
                {
                    Heat[i] = 0;
