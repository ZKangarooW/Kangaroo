...
#define Buzzer_Task_Priority 10//任务执行的优先级

#define Show_Stack_Depth 256//任务堆栈的堆栈
#define Show_Task_Priority 9//任务执行的优先级
...
#define Motor_Task_Priority 5//任务执行的优先级

#define Speed_Stack_Depth 128//任务堆栈的堆栈
#define Speed_Task_Priority 6//任务执行的优先级

#define Setting_Stack_Depth 128//任务堆栈的堆栈
#define Setting_Task_Priority 8//任务执行的优先级

#define Temp_Stack_Depth 128//任务堆栈的堆栈
#define Temp_Task_Priority 7//任务执行的优先级

#define Time_Stack_Depth 128//任务堆栈的堆栈
#define Time_Task_Priority 11//任务执行的优先级

#define Calibration_Stack_Depth 128//任务堆栈的堆栈
#define Calibration_Task_Priority 2//任务执行的优先级
...
TaskHandle_t Setting_Task_Handler;//函数句柄
TaskHandle_t Temp_Task_Handler;//函数句柄
TaskHandle_t Time_Task_Handler;//函数句柄
TaskHandle_t Calibration_Task_Handler;//函数句柄
...
void Start_Task(void *p)
{
    ...
    xTaskCreate((TaskFunction_t) Calibration_Task,
                (char *) "Calibration_Task",
                (configSTACK_DEPTH_TYPE) Calibration_Stack_Depth,
                (void *) NULL,
                (UBaseType_t) Calibration_Task_Priority,
    ...
    /* 创建设定任务 */
    xTaskCreate((TaskFunction_t) Setting_Task,
                (char *) "Setting_Task",
                (configSTACK_DEPTH_TYPE) Setting_Stack_Depth,
                (void *) NULL,
                (UBaseType_t) Setting_Task_Priority,
                (TaskHandle_t *)&Setting_Task_Handler);
                
    /* 创建控温任务 */
    xTaskCreate((TaskFunction_t) Temp_Task,
                (char *) "Temp_Task",
                (configSTACK_DEPTH_TYPE) Temp_Stack_Depth,
                (void *) NULL,
                (UBaseType_t) Temp_Task_Priority,
