...
float AD_T1[4];//ADC计算后的电压值
float AD_T2[4];//ADC计算后的电压值
float PT_VALUE_1_TEMP[4];//外部探头阻值
float PT_VALUE_2_TEMP[4];//台面探头阻值
float R_value1[4];//外部探头阻值
float R_value2[4];//台面探头阻值
...
void AFE_Sample_Handler(void)
{
    ...
    for(uint8_t i = 0;i<4;i++)
    {
		...
		R_value1[j] = (AD_T1[i]*R_3K)/(TL431_V-AD_T1[i])*1000;//乘1000是因为算出来是千欧为单位的，要转化成欧姆
		R_value2[j] = (AD_T2[i]*R_3K)/(TL431_V-AD_T2[i])*1000;//乘1000是因为算出来是千欧为单位的，要转化成欧姆
		PT_VALUE_1_TEMP[j]=R_value1[j] * (1000 /Param.Correct[j]);//计算电阻值
        PT_VALUE_2_TEMP[j]=R_value2[j] * (1000 /Param.Correct[j]);//计算电阻值
		if(PT_VALUE_2_TEMP[j] > 1758.56)
		{
			PT_VALUE_2_TEMP[j] = 1758.56f + (2156.08f-1758.56f)*((PT_VALUE_2_TEMP[j]-1758.56f)/(1940.98f-1758.56f));
		}
    }
}
