...
*****************************************************************
*/
void Heat_Out(int16_t Heat_Pwm[4])
{
    HEAT0 = (uint16_t)Heat_Pwm[0];
    HEAT1 = (uint16_t)Heat_Pwm[1];
    HEAT2 = (uint16_t)Heat_Pwm[2];
    HEAT3 = (uint16_t)Heat_Pwm[3];
}
...
static void Mesa_Ctrl(float dT, uint8_t i,int32_t Ctrl_temp)
{
	...
	AltPID_Calculation(dT, ctrl_val[i], adc_val[i], &Temp_Arg[i], &Temp_Val[i], 100, Out_Enable[i] * 400);
	if(Temp_Val[i].Out < 0)
		Temp_Val[i].Out = 0;
