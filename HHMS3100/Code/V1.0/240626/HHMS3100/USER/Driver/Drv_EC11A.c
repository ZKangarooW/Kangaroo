...
void EC11AKey_Scan(float dT)
{
	...
	if(EC11A[0].Key_Flag == 1)//按键被按下
	{
		...
		if(EC11A[0].Key_Cnt > 1.5 && EC11A[0].Key_Cnt < 3)//按键时间大于1.5S小于3S表示长按
		{
			if(EC11A[0].LongPress == 0)//如果没有一直一直长按着
			{
                ...
                {
                    Work_Num[i].Speed.Set = Work_Num[sys.New_Work].Speed.Set;
                    Work_Num[i].Temp.Set = Work_Num[sys.New_Work].Temp.Set;
                    Work_Num[i].Time.Set = Work_Num[sys.New_Work].Time.Set;
                }
                Twinkle_Time = 0;
                sys.SetMode_Option = 0;
                SetOK_Flag = 1;
	...
	if(EC11A[1].Key_Flag == 1)//按键被按下
	{
		...
		if(EC11A[1].Key_Cnt > 1.5 && EC11A[1].Key_Cnt < 3)//按键时间大于1.5S小于3S表示长按
		{
			...
			{
                if(sys.Run_Status != 1)//系统没启动的话
                {
                    sys.Run_Status = 1;//启动系统
                }
                else//系统启动的话
                {
                    Work_Num[0].Speed.ADDMode = 2;
                    Work_Num[1].Speed.ADDMode = 2;
                    Work_Num[2].Speed.ADDMode = 2;
                    Work_Num[3].Speed.ADDMode = 2;
