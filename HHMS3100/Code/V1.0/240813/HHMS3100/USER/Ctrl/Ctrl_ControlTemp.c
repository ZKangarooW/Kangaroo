...
void Temp_PID(void)
{
    for(uint8_t i = 0;i<4;i++)
    {
        ...
        Temp_Speed_Arg[i].Ki = 300*0.001f;//18
...
static void Mesa_Ctrl(float dT, uint8_t i,int32_t Ctrl_temp)
{
    ...
    if ((Ctrl_temp > 2000)&&(Work_Num[i].Temp.Rel < Ctrl_temp - 150))//达到目标温度-0.5℃前全速加热
    {
        Heat[i] = 399;
    }
    else if ((Ctrl_temp <= 2000)&&Work_Num[i].Temp.Rel < Ctrl_temp - 100)//达到目标温度-0.5℃前全速加热
    {
        T[i] += dT;
        if(T[i] <3.0f)
            Heat[i] = 250;
    ...
    else if ((Ctrl_temp > 2000)&&Work_Num[i].Temp.Rel < Ctrl_temp)//达到目标温度-0.5℃前全速加热
    {
        Heat[i] = 399;
    }
    else if (Work_Num[i].Temp.Rel >= Ctrl_temp + 10) //超出设置值1℃时全关
    {
        Heat[i] = 0;
    }
    else    //PID控制
    {   //设定加热周期为1000ms，1000分之PID_Para.pid_result毫秒开加热器
        if((Ctrl_temp - Work_Num[i].Temp.Rel > -50 && Ctrl_temp - Work_Num[i].Temp.Rel < 50))
            Out_Enable[i] = 1;//打开积分计算
        else
            Out_Enable[i] = 0;//关闭积分计算
        
        AltPID_Calculation(dT, Ctrl_temp, Work_Num[i].Temp.Rel, &Temp_Arg[i], &Temp_Val[i], 100, Out_Enable[i] * 100);
        if(Temp_Val[i].Out < 0)
            Temp_Val[i].Out = 0;
        if(Temp_Val[i].Out > 100)
            Temp_Val[i].Out = 100;
        
        if(Ctrl_temp > Work_Num[i].Temp.Rel)
        {
...
void Temp_UpSpeed(float dT)
{
    ...
    for(uint8_t i = 0;i<4;i++)
    {
        ...
        if(T[i] > 1.0f)
        {
            Work_Num[i].Temp.Speed = Work_Num[i].Temp.Rel - Work_Num[i].Temp.Old;
            Work_Num[i].Temp.Old = Work_Num[i].Temp.Rel;
            T[i] = 0;
        }
    }
}
