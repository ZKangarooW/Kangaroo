...
static void Water_Ctrl(float dT, int32_t Ctrl_temp)
{
    if(Temp.Up_Speed == 1)
    {
        ...
        W_Temp_Arg.Kd = 900*0.001f;;//控水温
        if(Temp.Rel<Temp.Ctrl-30)
        {                                                                                                                                                                               
            set_Mesa_power(dT,Temp.Ctrl*3.5);
        } 
        else
        {
            IncPID_Calculation(dT, Ctrl_temp, Temp.Outside, &W_Temp_Arg, &W_Temp_Val,30, 300);
    ...
    {
        W_Temp_Arg.Kp = 800*0.001f;
        W_Temp_Arg.Ki = 8*0.001f;//18
        W_Temp_Arg.Kd = 90*0.001f;//控水温
        if(Temp.Rel<Temp.Ctrl-60)
        {
            IncPID_Calculation(dT, Ctrl_temp, Temp.Outside, &W_Temp_Arg, &W_Temp_Val,30, 300);
            set_Mesa_power(dT,Temp.Ctrl*1.7+W_Temp_Val.Out);
        } 
        else
        {
            IncPID_Calculation(dT, Ctrl_temp, Temp.Outside, &W_Temp_Arg, &W_Temp_Val,30, 300);
    ...
    else
    {
        ...
        {
            IncPID_Calculation(dT, Ctrl_temp, Temp.Outside, &W_Temp_Arg, &W_Temp_Val,30, 300);
            set_Mesa_power(dT,Temp.Ctrl*2.0+W_Temp_Val.Out);
        } 
        else
        {
            IncPID_Calculation(dT, Ctrl_temp, Temp.Outside, &W_Temp_Arg, &W_Temp_Val,30, 300);
