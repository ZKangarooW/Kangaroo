...
/**********全局变量声明******/
float Speed_Twinkle_Time,Temp_Twinkle_Time,Time_Twinkle_Time,Pmode_Twinkle_Time;//速度闪烁时间、温度闪烁时间和时间闪烁时间

/**********局部变量声明******/
uint8_t Tab[] = {0xFA,0x0A,0xD6,0x9E,0x2E,0xBC,0xFC,0x1A,0xFE,0xBE};//0·9
uint8_t Temp_ShowFlag,Speed_ShowFlag,Time_ShowFlag,Pmode_ShowFlag;//温度、速度、时间显示的标志位 0:常亮 1：熄灭
...
{
	static float Speed_T,Temp_T,Time_T,PMode_T;
...
//				Beep_Time = 0.1;//蜂鸣器响0.1S
    ...
    if(Pmode_Twinkle_Time && !EC11A[0].EC11A_Knob)//时间闪烁和没有操作按键时
    {
		PMode_T += dT;
		if(PMode_T >= 0.5f)
		{
			...
			{
				SpeedSet_Flag = TempSet_Flag = TimeSet_Flag = 1;//进入时间设定
//				Beep_Time = 0.1;//蜂鸣器响0.1S
			} 
			PMode_T = 0;			
		}			
...
void Display_Temp(int16_t dis_set_temp,int16_t dis_rel_temp)
{
	...
	if(sys.Display)
    {
        Temp_QU = Temp_BU = Temp_SU = Temp_GU = 0x00;
        seg8 &= 0x7F;seg8 |= 0x80;//℃
        seg6 &= 0xFE;seg6 |= 0x00;//实际温度的小数点
        ...
        if(Temp.Mesa > 500)
        {
            if(Temp.Mesa > 999)//大于999时
            {
                Val=Temp.Mesa/1000;//取出千位
                Temp_QD = Tab[Val];
            }
            else
            {
            ...
            if(Temp.Mesa > 99)//大于99时
            {
                Val=Temp.Mesa/100;//取出百位
                if(Temp.Mesa > 999)//大于999时
                    Val=Val%10;//取出百位
                Temp_BD = Tab[Val];
            }
...
void Display_Speed(int16_t dis_set_speed,int16_t dis_rel_speed)
{
	...
	if(sys.Display)
    {
        Speed_QD=Speed_BD=Speed_SD=Speed_GD=0x00;
        seg15 &= 0x7F;seg15 |= 0x00;//rpm
        Speed_QU = 0;
        if(Hot_ShowFlag==0)
        {
            Speed_BU = 0x00;//H
...
*****************************************************************
*/
void Display_Time(int32_t dis_time,uint8_t dis_option)
{
    ...
    if(sys.Display)
    {
        Time_Q=Time_B=Time_S=Time_G=0X00;
...
void DownTime_Display(void)
{
    ...
    uint8_t seg3=0;
    uint8_t seg4=0;
    uint8_t Temp_QU,Temp_BU;//实际温度的计算位数取值
    ...
    /*************发送数据***************/	
	Write_Addr_Dat_N(16, 0x00, 1);//SEG19
	Write_Addr_Dat_N(18, 0x00,1);//SEG18
	Write_Addr_Dat_N(20, 0x00,1);//SEG17
	Write_Addr_Dat_N(38, 0x00,1);//SEG8
	Write_Addr_Dat_N(40, 0x00,1);//SEG7
	Write_Addr_Dat_N(42, 0x00,1);//SEG6
	Write_Addr_Dat_N(44, 0x00,1);//SEG5
	Write_Addr_Dat_N(46, 0x00,1);//SEG4
	Write_Addr_Dat_N(48, 0x00,1);//SEG3
	Write_Addr_Dat_N(50, 0x00,1);//SEG2
	Write_Addr_Dat_N(52, 0x00,1);//SEG1
    Write_Addr_Dat_N(22, 0x00,1);//SEG16
	Write_Addr_Dat_N(24, 0x00,1);//SEG15
	Write_Addr_Dat_N(26, 0x00,1);//SEG14
	Write_Addr_Dat_N(28, 0x00,1);//SEG13
	Write_Addr_Dat_N(30, 0x00,1);//SEG12
	Write_Addr_Dat_N(32, 0x00,1);//SEG11
	Write_Addr_Dat_N(34, 0x00,1);//SEG10
	Write_Addr_Dat_N(36, 0x00,1);//SEG9	
    Write_Addr_Dat_N(0, seg1,1);//SEG27
	Write_Addr_Dat_N(2, seg2,1);//SEG26
	Write_Addr_Dat_N(4, seg3,1);//SEG25
	Write_Addr_Dat_N(6, seg4,1);//SEG24
	Write_Addr_Dat_N(8, 0x00,1);//SEG23
	Write_Addr_Dat_N(10, 0x00,1);//SEG22
	Write_Addr_Dat_N(12, 0x00,1);//SEG21
	Write_Addr_Dat_N(14, 0x00,1);//SEG20
}
...
void Show_Display(void)
{
    ...
    else
    {
        ...
        Display_Time(Time.Display,PMode.Option);//显示时间
    }
}
