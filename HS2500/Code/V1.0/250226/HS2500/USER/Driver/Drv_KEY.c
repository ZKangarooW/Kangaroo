...
void Key_Scan(float dT)
{
	...
	if(Key_Flag1 == 1)//按键被按下
	{
		...
		{   
			if(Key_Cnt1 > 0.1 && Key_Cnt1 < 1.5)//小于1.5S是单击
			{
				...
				{
                    if (PMode.Show_Circle == 1) // 如果打开P模式
                    {
                        if(Pmode_Twinkle_Time)
                        {
                            PMode.Show_Circle = 0; // 关闭P模式
                            PMode.Option = 0;
                            Param_Read();//读取参数
                    ...
                    else if (PMode.Show_Circle == 0)
                    {
                        PMode.Show_Circle = 1; // 打开P模式
                        PMode.Option = 1;
                        Param_Read();   // 读取参数
                        Speed_Twinkle_Time = Temp_Twinkle_Time = Time_Twinkle_Time = 0;
                        Pmode_Twinkle_Time = 2.0f;
    ...
    if(HAL_GPIO_ReadPin(Key_POR_GPIO_Port,Key_POR_Pin) == 1)//按下按键
	{	
		sys.Display = 1;
        /*推出设置界面*/
