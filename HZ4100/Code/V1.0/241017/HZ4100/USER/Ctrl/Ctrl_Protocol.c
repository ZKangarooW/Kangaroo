...
/*
*****************************************************************
 * 函数原型：void Data_Receive_Anl(uint8_t *data_buf, uint8_t num)
 * 功    能：Data_Receive_Anl函数是协议数据解析函数，函数参数是符合协议格式的一个数据帧，
			 该函数会首先对协议数据进行校验，校验通过后对数据进行解析，实现相应功能
 * 输    入: data_buf：数据，num：长度
 * 参    数：uint8_t *data_buf, uint8_t num
*****************************************************************
*/
