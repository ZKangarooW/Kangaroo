...
/**********局部变量声明******/
uint16_t WS2812_Buf[RESET_WORD+RGB_NUM * 24+DUMMY_WORD]; // 存放的数组
...
void WS2812_Update(void)
{
	...
	{
		HAL_TIM_PWM_Start_DMA(WS2812[0].Tim, WS2812[0].CHANNEL,(uint32_t *)&WS2812_Buf[0], RESET_WORD+RGB_NUM * 24+DUMMY_WORD); // 开始传输字节数据
...
void WS2812_SetColor(uint8_t num, uint8_t r, uint8_t g, uint8_t b,float light)
{
	...
	else if (WS2812_Status[0].Number <= RGB_NUM) // 只改变指定序号灯的颜色，看RGB_NUM的值
	{
...
void WS2812_Init(void)
{
	...
	WS2812[0].CHANNEL = TIM_CHANNEL_3; // 定时器通道数
    WS2812_SetColor(0xFF, 0x00, 0x00, 0x00, 0.6f);
}
...
void KEYLED_Check(float dT)
{
	if (Lock_Switch) // 按下按键
	{
		WS2812_SetColor(0x00, 0xFF, 0xFF, 0xFF, 0.6f);
	}
	else
	{
		WS2812_SetColor(0x00, 0x00, 0x00, 0x00, 0.6f);
	}

	if (Shake_Switch) // 按下按键
	{
		WS2812_SetColor(0x01, 0xFF, 0xFF, 0xFF, 0.6f);
	}
	else
	{
		WS2812_SetColor(0x01, 0x00, 0x00, 0x00, 0.6f);
	}
}
...
void ShakeRGB_Check(float dT)
{
	...
	{
	case 0:
		WS2812_SetColor(0xFF, 0xFF, 0x00, 0x00, 0.6f);
		break;
	case 1:
		WS2812_SetColor(0xFF, 0x00, 0x00, 0x00, 0.6f);
		break;
...
void WS2812_Duty(float dT)
{
	...
	if(Shake_Alarm == 0)
	{
		if(sys.Run_Status == 1)
		{
			WS2812_SetColor(0xFF, 0xFF, 0xFF, 0xFF, 0.6f);
			KEYLED_Check(dT);
			time += 20;
			step = (time / 50) % 16+2;
			val1 = step;
			if(val1 > 16) val1 = 2;
			WS2812_SetColor(val1, 0xFF, 0xFF, 0xFF, 0.6f);
			val2 = val1+1;
			if(val2 > 17) val2 = 2;
			WS2812_SetColor(val2, 0xFF, 0xFF, 0xFF, 0.6f);	
			val3 = val2+1;
			if(val3 > 17) val3 = 2;
			WS2812_SetColor(val3, 0xFF, 0xFF, 0xFF, 0.6f);	
			val4 = val3+1;
			if(val4 > 17) val4 = 2;
			WS2812_SetColor(val4, 0xFF, 0xFF, 0xFF, 0.6f);
		}
		else
		{
			WS2812_SetColor(0xFF, 0xFF, 0xFF, 0xFF, 0.6f);
