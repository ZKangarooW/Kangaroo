...
void Display_Show(uint16_t dis_rel_speed, uint16_t dis_set_speed, uint32_t dis_time, int16_t dis_temp)
{
    ...
    if (!Shake_Alarm)
    {
        ...
        {
            bit_Cal(6, 0x07, 0x00);bit_Cal(7, 0x0F, 0x00);//不显示
            bit_Cal(4, 0x0E, 0x00);bit_Cal(5, 0x1E, 0x00);//不显示
            bit_Cal(2, 0x0E, 0x00);bit_Cal(3, 0x1E, 0x00);//不显示
            bit_Cal(0, 0x07, 0x00);bit_Cal(1, 0x0F, 0x00);//不显示
            /**********时间冒号图标**********/
            bit_Cal(4, 0x01, 0x00);
        }
        /**********温度显示**********/
...
void Deal_Speed(float dT)
{
	...
	else
	{
		if (Speed.ADDMode == 2) // 速度下降模式下
		{
			...
			{
				if (Speed.Display_Rel > Speed.Rel)
					Speed.Display_Rel -= 500; // 显示当前速度
				if(Speed.Display_Rel >= Speed.Ctrl)
					Speed.Display_Rel = Speed.Ctrl;
				if(Speed.Display_Rel <= 0)
					Speed.Display_Rel = 0;
			}
			else // 当前速度大于上一次速度
			{
