...
void Display(int16_t speed,int32_t time)
{
    ...
    else if(CGF.Mode == 1)//在CGF模式下，和实际速度显示“CGF“和时间显示“-----”
    {
        seg1&=0x80;seg1|=0x00;//不显示
        seg2&=0x80;seg2|=0x53;//显示"C"
        seg3&=0x80;seg3|=0x7B;//显示"G"
        seg4&=0x80;seg4|=0x1B;//显示"F"
    ...
    else if(CGF.Mode == 2)//在PRP模式下，和实际速度显示“PRP“和时间显示“-----”
    {
        seg1&=0x80;seg1|=0x00;//不显示
        seg2&=0x80;seg2|=0x1F;//显示"P"
        seg3&=0x80;seg3|=0x1A;//显示"R"
        seg4&=0x80;seg4|=0x1F;//显示"P"
    ...
    else if(CGF.Mode == 3)//在APRF模式下，和实际速度显示“APRF“和时间显示“-----”
    {
        seg1&=0x80;seg1|=0x3F;//显示"A"
        seg2&=0x80;seg2|=0x1F;//显示"P"
        seg3&=0x80;seg3|=0x1A;//显示"R"
        seg4&=0x80;seg4|=0x1B;//显示"F"
    ...
    else if(CGF.Mode == 4)//在IPRF模式下，和实际速度显示“IPRF“和时间显示“-----”
    {
        seg1&=0x80;seg1|=0x12;//显示"I"
        seg2&=0x80;seg2|=0x1F;//显示"P"
        seg3&=0x80;seg3|=0x1A;//显示"R"
        seg4&=0x80;seg4|=0x1B;//显示"F"
...
//					Speed.Display += 1;//显示当前速度
			...
			if(sys.Motor_Stop == 0)
			{
                if(CGF.Mode == 0)//在CGF模式下，和实际速度显示“CGF“和时间显示“-----”
                {
                ...
                {
                    if(Speed.Display >= Param.C_Param[CGF.Option][1])//实际速度大于等于控制速度
                    {
                        Speed.ADDMode = 3;//进入稳定模式
                        return;
                    }
...
//					Speed.Display -= 1;//显示当前速度
        ...
        else if(Speed.ADDMode == 3)//速度稳定模式下
        {
            ...
            {
                Speed.Display = Param.C_Param[CGF.Option][1];//显示控制速度
            }
