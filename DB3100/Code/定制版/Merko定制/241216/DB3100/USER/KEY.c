...
uint8_t setting_Mode;//进入设置模式

void check_set(float dT)
{
    if(UPPress_Time >=5.0f && DownPress_Time >= 5.0f)
    {
        if(setting_Mode == 0)
        {
            setting_Mode = 1;
            Select_Option = 1;//设置选项切换
            if(Set_Mode_Enable == 0)
            {
                ...
                Twinkle_On = 3000;//闪烁倒计时，如果停止按键设置，6S后停止闪烁
    ...
    if (Key_Flag3 == 1) // 按键被按下
    {
        ...
        else if (Key_Cnt3 > 1.9f && Key_Cnt3 < 2.1f) // 按键时间大于1.9S小于2.1S表示长按
        {
            ...
            else
            {
                ...
                else if(Select_Option == 1 && UPPress_Time == 0)//在设置模式选项
                {
                    run_mode--;//P记忆位置--
                    if(run_mode < 1)//小于1时返回到第九个位置
                    {
    ...
    if (Key_Flag2) // 按键被按下
    {
        ...
        else if (Key_Cnt2 > 1.9f && Key_Cnt2 < 2.1f) // 按键时间大于1.9S小于2.1S表示长按
        {
            ...
            else
            {
                ...
                else if(Select_Option == 1 && DownPress_Time == 0)//在设置模式选项
                {
                    run_mode++;//P记忆位置++
                    if(run_mode > 9)//大于9时返回到第一个位置
                    {
                ...
                else
                {
                    if(setting_Mode)
                    {
                        ...
                        else
                        {
                            if(Select_Option > 3)//在温度和时间和P模式来换选择
                            {
                                Select_Option = 1;//不进行设置    	
                            }
                            Twinkle_On = 3000;//闪烁倒计时，如果停止按键设置，6S后停止闪烁
                            Beep_Time = 0.1;//蜂鸣器响0.1S
                        }
                    }
                    else if(Set_Mode_Enable)
                    {
                        Select_Option = 1;
                        Twinkle_On = 3000;//闪烁倒计时，如果停止按键设置，6S后停止闪烁
                        Beep_Time = 0.1;//蜂鸣器响0.1S
