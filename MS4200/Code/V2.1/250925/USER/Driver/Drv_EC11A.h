...
/**********宏定义************/
#define EC11A_FastSpeed 100//旋钮快转的值

//用于识别按键按下的状态识别 
#define KEY_DOWN GPIO_PIN_RESET/*键按下*/
#define KEY_UP  GPIO_PIN_SET/*键弹起*/

/**********结构体************/
typedef struct
{
	uint16_t EXTI_Pin;//EC11A旋钮中断引脚
	uint16_t EC11A_A_Pin;//EC11A旋钮输入引脚
	GPIO_TypeDef* EC11A_A_GPIO;//EC11A旋钮输入GPIO端口
	uint16_t EC11A_B_Pin;//EC11A旋钮输入引脚
	GPIO_TypeDef* EC11A_B_GPIO;//EC11A旋钮输入GPIO端口
	...
	uint8_t Falling_Cnt; //判断是否先经过下降沿触发中断
    uint8_t B_Level_Old; //B相上一次电平
    uint8_t B_Level_Now; //B相当前电平
}_EC11A_;
...
void Check_Knob(float dT);//检测旋钮的状态-500ms
void EC11A_EXTI_Interrupt(uint16_t GPIO_Pin);//EC11A旋钮中断函数
