...
void Lcd_Init(void)
{
	...
	if(Param.type == 0)
	{
		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 135);//不输出PWM
	}
	else if(Param.type == 1)
	{
		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 95);//不输出PWM
	}
...
void Lcd_All(void)
{
	...
	Write_Addr_Dat_N(0,0x3F,1);//SEG9不显示+TOUCH
	Write_Addr_Dat_N(5,0x7F,1);//SEG12不显示RUN
	Write_Addr_Dat_N(7,0x7F,1);//SEG13不显示方块
	Write_Addr_Dat_N(9,0x7F,1);//SEG14不显示园
}
