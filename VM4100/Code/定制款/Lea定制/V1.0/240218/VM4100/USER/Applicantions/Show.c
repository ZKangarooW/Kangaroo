...
void Check_ShowFlag(float dT)
{
    ...
    if(sys.SetMode_Option && Key_Status==0)//闪烁和没有操作旋钮时
    {
        T += dT;//闪烁计时
		if(T >= 0.5f)//大于0.5S后
		{
			if(sys.SetMode_Option == 1 ||sys.SetMode_Option == 3)//设置速度
			{
				Speed_ShowFlag = ~Speed_ShowFlag;//速度闪烁
				Time_ShowFlag = 0;//时间常亮
			}
			else if(sys.SetMode_Option == 2||sys.SetMode_Option == 4)//设置时间
			{
...
void Deal_Speed(void)
{
    ...
    if(sys.Run_Status == 1)//启动的情况下
    {
        if(Speed_ADDMode == 0)//在电机控制中，速度未处理
        {
            ...
            {
                Speed_New=0;//现在的速度清零
                Speed_Last = Rel_Speed;//之前的速度等于当前显示速度
