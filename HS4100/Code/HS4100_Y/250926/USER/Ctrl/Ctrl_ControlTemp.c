...
void Temp_PID(void)
{
	...
	Temp_Out_Arg.Kp = 500*0.001f;
	Temp_Out_Arg.Ki = 40*0.001f;//40
	Temp_Out_Arg.Kd = 100*0.001f;//控台面
}
...
static void Mesa_Ctrl(float dT,int32_t Ctrl_temp)
{
	...
	if(Ctrl_temp > 3100) Ctrl_temp = 3100;//最高加热Temp_MAX度
...
*/
float final_output;
float no_progress_count = 0;
float last_good_temp = 0;
uint8_t pid_active = 0;   
float base_output = 0;
float pid_output = 0;
static void Outside_Ctrl(float dT, int32_t Ctrl_temp)
{
	...
	// 基础分段控制
    if(error >= 150) base_output = Ctrl_temp * 2.0f;
    else if(error > 100) base_output = Ctrl_temp * 1.1f;
    else if(error >= 5) base_output = Ctrl_temp * 1.0f;
    else if(error >= 0) base_output = Ctrl_temp * 1.0f;
    else base_output = 0;
	
	// 智能判断是否需要PID
    if (error > 20.0f && error < 100.0f) 
	{
        // 检测温度是否有进展
		...
		{
            no_progress_count+=dT;
        }
        
        // 如果10秒没有进展，启用PID
        if (no_progress_count > 10.0f) 
		{
            pid_active = 1;
        }
    } 
