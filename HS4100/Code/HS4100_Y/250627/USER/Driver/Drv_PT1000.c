...
float AD_T[2];//ADC计算后的电压值
float R_value[2];//探头阻值
void AFE_Sample_Handler(void)
{
	ADC_Val_Avg[0] = Filter_ADC(0);
	ADC_Val_Avg[1] = Filter_ADC(1);
	
	AD_T[0]=((float)ADC_Val_Avg[0]*Vref_3V3/4096)/OP_Value+(R_910R/(R_3K+R_910R)*TL431_V);//计算电压值
	AD_T[1]=((float)ADC_Val_Avg[1]*Vref_3V3/4096)/OP_Value+(R_910R/(R_3K+R_910R)*TL431_V);//计算电压值
	
	R_value[0] = (AD_T[0]*R_3K)/(TL431_V-AD_T[0])*1000;//乘1000是因为算出来是千欧为单位的，要转化成欧姆
	R_value[1] = (AD_T[1]*R_3K)/(TL431_V-AD_T[1])*1000;//乘1000是因为算出来是千欧为单位的，要转化成欧姆
	
	PT_VALUE_2_TEMP=R_value[0] * (1000 /Param.Correct);//计算电阻值
	PT_VALUE_1_TEMP=R_value[1] * (1000 /Param.Correct);//计算电阻值
}
