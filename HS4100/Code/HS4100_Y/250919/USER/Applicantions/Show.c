	...
	else if(sys.Run_Status== 1 && Temp.Ctrl)
	{
		...
		{ 
			...
			{	//只有在实际温度大于90℃，控制温度大于95小于100℃，插入外部探头才有作用
				if(Temp.Rel > 900 && Temp.Ctrl >= 950 && Temp.Ctrl <= 1000 && PT_VALUE_1_TEMP < 2200)
				{
					T += dT;
					if(T > 10.0f)
					{
                ...
                else if(Temp.Rel>=2800 && Temp.Ctrl>=2800 && PT_VALUE_1_TEMP >= 2200)
                {
                    T += dT;
					if(T > 2.0f)
					{
			...
			if(Temp.Display_Rel + 20 >= Temp.Ctrl)//当前的温度大于等于控制温度
			{
				Temp.ADDMode = 3;//进入稳定模式
			}
		...
		{ 
			...
			if(Temp.Display_Rel - 20 <= Temp.Ctrl)//当前的温度小于等于控制温度
			{
				Temp.ADDMode = 3;//进入稳定模式
			}
		...
		else if(Temp.ADDMode == 3)//温度稳定模式下
		{
            ...
            else
            {
				...
				Temp_Out_Val.Integral = Temp_Out_Val.Integral * 0.6f;
				Temp_Arg.Kd = 10;//pid参数调整
                T1 = 0;
            }
		}			
