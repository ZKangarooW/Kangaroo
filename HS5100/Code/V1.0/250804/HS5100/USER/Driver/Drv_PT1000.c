...
//	PT_VALUE_2_TEMP = Filter_ADC_EMA(Filter_PT2,0);//计算阻值台面
	...
	Filter_PT1 = ADC_Val[0];//读取并滤波PT1000的ADC值外部
    PT_VALUE_1_TEMP = Filter_ADC_EMA(Filter_PT1,0) * (1000 /Param.Correct);//计算阻值外部
    
    Filter_PT2 = ADC_Val[1];//读取并滤波PT1000的ADC值台面
    PT_VALUE_2_TEMP = Filter_ADC_EMA(Filter_PT2,1) * (1000 /Param.Correct);//计算阻值台面
