...
void EC11A_Init(void)
{
	...
	// 初始化消抖相关变量
	EC11A[0].Debounce_Flag = 0;
	EC11A[0].Debounce_Cnt = 0;
	...
	// 初始化消抖相关变量
	EC11A[1].Debounce_Flag = 0;
	EC11A[1].Debounce_Cnt = 0;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	{
		if(EC11A[0].Debounce_Flag == 0)//检测到电平变化
		{
			EC11A[0].Debounce_Flag = 1;// 激活延时
			EC11A[0].Debounce_Cnt = 0;// 重置计数器
		}
	}
	...
	{
		if(EC11A[1].Debounce_Flag == 0)//检测到电平变化
		{
			EC11A[1].Debounce_Flag = 1;// 激活延时
			EC11A[1].Debounce_Cnt = 0;// 重置计数器
		}
	}
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	{
		// 处理消抖
		if(EC11A[0].Debounce_Flag)
		{
			if(++EC11A[0].Debounce_Cnt >= EC11A_DEBOUNCE_DELAY)//延时50ms
			{
				if(sys.Display)
				{
					return;
				...
				if(HAL_GPIO_ReadPin(EC1A_GPIO_Port,EC1A_Pin) == 1)//A上升沿触发外部中断
				{
	...
	{
		// 处理消抖
		if(EC11A[1].Debounce_Flag)
		{
			if(++EC11A[1].Debounce_Cnt >= EC11A_DEBOUNCE_DELAY)//延时50ms
			{
				if(sys.Display)
				{
					return;
				...
				if(HAL_GPIO_ReadPin(EC2A_GPIO_Port,EC2A_Pin) == 1)//A上升沿触发外部中断
				{
