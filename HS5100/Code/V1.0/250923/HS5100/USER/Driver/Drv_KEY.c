...
void Key_Scan(float dT)
{
	...
	if(Key_Flag2 == 1)//按键被按下
	{
		...
		if(Key_Cnt2 > 1.5 && Key_Cnt2 < 3)//按键时间大于1.5S小于3S表示长按
		{
			if(LongPress2 == 0)//如果没有一直一直长按着
			{
                ...
                else if(PT_VALUE_2_TEMP > 980 && PT_VALUE_2_TEMP <1020)
                {
                    Calibration.Switch = 1;    // 强制开启校准模式
					Param.Calibration = 0;     // 重置校准标志
					Param.Correct[0] = 1000.0f; // 重置1kΩ校准值
					Param.Correct[1] = 2000.0f; // 重置2kΩ校准值
    ...
    if(HAL_GPIO_ReadPin(Key_POR_GPIO_Port,Key_POR_Pin) == KEY_UP)//按下按键
	{	
		if(sys.Display == 0)
        {
			sys.Display = 1;
