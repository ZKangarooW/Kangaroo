#include "Param.h"

/**********结构体**********/
struct _Save_Param_ Param;//原始数据

/**********全局变量声明******/
uint8_t Save_Param_En;

/*
*****************************************************************
 * 函数原型：void Param_Reset(void)
 * 功    能：初始化硬件中的参数
*****************************************************************
*/
void Param_Reset(void)
{
	Param.Flash_Check_Start = FLASH_CHECK_START;
	
    for (uint8_t i = 0; i <= 9; i++)
    {
        Param.P_Param[i][0] = 370; // 温度
        Param.P_Param[i][1] = 100; // 速度
        Param.P_Param[i][2] = 0;   // 时间
    }
    
	Param.Correct[0] = 1000.0f;//1K电阻值
	Param.Correct[1] = 2000.0f;//2K电阻值
	Param.Calibration = 0;//没有校准过
    
    Param.Unit = 0;//温度单位
    Param.TempUp_Speed = 0;//上升速度
    Param.SpeedUp_Speed = 0;//上升速度
    Param.Safe_Temp = Temp_MAX + 500;//安全温度
    Param.DownTime_Type = C1;//默认为C1
    
	Param.Flash_Check_End  = FLASH_CHECK_END;
}

/*
*****************************************************************
 * 函数原型： void Param_Save(void)
 * 功    能： 保存硬件中的参数
*****************************************************************
*/
void Param_Save(void)
{
	Flash_Write((uint8_t *)(&Param),sizeof(Param));
}

/*
*****************************************************************
 * 函数原型：void Param_Read(void)
 * 功    能：读取硬件中的参数，判断是否更新
*****************************************************************
*/
void Param_Read(void)
{
	Flash_Read((uint8_t *)(&Param),sizeof(Param));
	
	//板子从未初始化
	if(Param.Flash_Check_Start != FLASH_CHECK_START || Param.Flash_Check_End != FLASH_CHECK_END)
	{
		Param_Reset();
		Speed.Set = Param.P_Param[PMode.Option][1];//将Flash中的速度赋值
		Temp.Set = Param.P_Param[PMode.Option][0];//将Flash中的温度赋值
		Time.Set = Param.P_Param[PMode.Option][2]; //将Flash中的时间赋值
        
        Temp.Unit = Param.Unit;//温度单位
        Temp.Up_Speed = Param.TempUp_Speed;//上升速度
        Speed.Up_Speed = Param.SpeedUp_Speed;//上升速度
        Temp.Safe_Temp = Param.Safe_Temp;//安全温度
        
        sys.DownTime_Type = Param.DownTime_Type;//将Flash中的倒计时类型赋值
        
		SpeedSet_Flag = TempSet_Flag = TimeSet_Flag=1;//进入设置
		Save_Param_En = 1;
	}
	else
	{
		Speed.Set = Param.P_Param[PMode.Option][1];//将Flash中的速度赋值
		Temp.Set = Param.P_Param[PMode.Option][0];//将Flash中的温度赋值
		Time.Set = Param.P_Param[PMode.Option][2]; //将Flash中的时间赋值
        Temp.Unit = Param.Unit;//温度单位
        Temp.Up_Speed = Param.TempUp_Speed;//上升速度
        Speed.Up_Speed = Param.SpeedUp_Speed;//上升速度
        Temp.Safe_Temp = Param.Safe_Temp;//安全温度
        
        sys.DownTime_Type = Param.DownTime_Type;//将Flash中的倒计时类型赋值
        
		SpeedSet_Flag = TempSet_Flag = TimeSet_Flag=1;//进入设置
	}
	
	//保存参数
	if(Save_Param_En)
	{
		Save_Param_En = 0;
		Param_Save();
	}
}

/*
*****************************************************************
 * 函数原型：void Param_Save_Overtime(float dT)
 * 功    能：保存标志位置1，0.5s后保存
*****************************************************************
*/
void Param_Save_Overtime(float dT)
{
	static float time;
	
	if(Save_Param_En)
	{
		time += dT;
		
		if(time >= 0.5f)
		{
			Param_Save();
			Save_Param_En = 0;
		}
	}
	else 
		time = 0;
}
