...
void AFE_Sample_Handler(void)
{
	...
	temp_correct1 = 1.0044f;
	
	ADC_Val_Avg[0] = Filter_ADC();
	ADC_Val_Avg[1] = Filter_ADC1();
	AD_T1=((float)ADC_Val_Avg[0]*Vref_3V3/4096)/OP_Value/Vref+K1;//计算电压值
	PT_VALUE_1_TEMP=3000*AD_T1/(1-AD_T1)*temp_correct;//计算电阻值
	AD_T2=((float)ADC_Val_Avg[1]*Vref_3V3/4096)/OP_Value/Vref+K1;//计算电压值
...
//    Temp.Mesa = AFE_GetTemperature(value1);//台面温度
	T += dT;
	AFE_Sample_Handler();//计算阻值
    
	if(T >= 1.0f)
	{
        if(PT_VALUE_1_TEMP < 2419)//假如插入外部探头
