...
/**********全局变量**********/
uint16_t PT_VALUE_1_TEMP;//外部PT1000探头阻值
uint16_t PT_VALUE_2_TEMP;//台面PT1000探头阻值
...
{
    uint16_t adc_raw_vrefint = ADC_Val[2];    // 内部参考电压ADC值
...
{
    ADC_Val_Avg[0] = ADC_MultiChannel_Filter(1, ADC_Val[1]);// 台面温度ADC值滤波
    ADC_Val_Avg[1] = ADC_MultiChannel_Filter(0, ADC_Val[0]);//外部温度ADC值滤波
