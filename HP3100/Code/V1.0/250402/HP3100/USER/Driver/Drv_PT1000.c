...
/**********局部变量**********/
uint16_t ADC_Val[AD_LEN];//adc的值 0:台面温度ad值 1：TL431的ad值 2：单片机内部的电压AD（参考是1.2f）
...
*****************************************************************
*/
float Filter_ADC_EMA(float AD_in)
{   
    static float EMA_value;//EMA滤波后的值，因为需要更新所以用静态
    float V_out;//电压值
    float R_value;//电阻值
...
void Read_Temp(float dT)
{
    ...
    float Filter_PT;//滤波后的PT1000的ADC值
	
	T += dT;
    
    Vref_AD = Filter_ADC(2);//读取并滤波单片机内部电压的ADC值
    TL431_AD = Filter_ADC(1);//读取并滤波TL431电压的ADC值
    TL431_V = (TL431_AD / Vref_AD) * Vref;//计算TL431的电压值
    
    Filter_PT = Filter_ADC(0);//读取并滤波PT1000的ADC值
    PT_VALUE_1_TEMP = Filter_ADC_EMA(Filter_PT);//计算阻值
