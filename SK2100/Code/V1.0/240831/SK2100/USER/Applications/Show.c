...
void Display(int16_t speed,int32_t time)
{
	...
	/***********时间计算**********/
    SH = time / 3600 / 10;      // 计算十位为单位的小时数
    H = time / 3600 % 10;       // 计算个位为单位的小时数
    SM = time % 3600 / 60 / 10; // 计算十分位为单位的分钟数
    M = time % 3600 / 60 % 10;  // 计算个分位为单位的分钟数
