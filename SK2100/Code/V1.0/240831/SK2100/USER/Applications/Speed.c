...
void Encoder_Init(void)
{
    ...
    HAL_TIM_IC_Start_IT(&htim1, TIM_CHANNEL_1); // 开启time1通道1输入捕获
}
...
float rel1;
/**
 * @brief Tim1通道1的输入捕获回调函数
 *
 */
void TIM1CaptureChannel1Callback(void)
{
    capture1 = __HAL_TIM_GET_COMPARE(&htim1, TIM_CHANNEL_1); // 获取Tim14通道1的输入捕获
    if (capture1 > capture2)
        capture = capture1 - capture2;
    else
        capture = capture1 + (0xFFFF - capture2);
    rel1 = 10000.0f / (float)capture /4; // 计算速度,齿比1：4
    capture2 = capture1;
