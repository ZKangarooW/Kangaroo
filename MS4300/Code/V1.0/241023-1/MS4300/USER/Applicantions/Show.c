...
/**********全局变量声明******/
float Speed_Twinkle_Time,Time_Twinkle_Time,Pmode_Twinkle_Time;//速度闪烁时间和时间闪烁时间

/**********局部变量声明******/
uint8_t TimeIcn_ShowFlag,SpeedIcn_ShowFlag,Pmode_ShowFlag;//时间图标闪烁和速度图标闪烁
uint8_t Speed_ShowFlag,Time_ShowFlag,Lock_ShowFlag;//速度、时间显示的标志位 0:常亮 1：熄灭
...
static void Check_ShowFlag(float dT)
{
	...
	if(Speed_Twinkle_Time && EC11A[1].EC11A_Knob<=0)//速度闪烁和没有操作按键时
    {
		Speed_T += dT;
		if(Speed_T >= 0.5f)
		{
...
//				Beep_Time = 0.1;//蜂鸣器响0.1S
	...
	if(Time_Twinkle_Time && EC11A[0].EC11A_Knob<=0)//时间闪烁和没有操作按键时
    {
		Time_T += dT;
		if(Time_T >= 0.5f)
		{
...
//				Beep_Time = 0.1;//蜂鸣器响0.1S
    ...
    if(Pmode_Twinkle_Time && (EC11A[1].EC11A_Knob<=0 && EC11A[0].EC11A_Knob<=0))//时间闪烁和没有操作按键时
    {
		Pmode_T += dT;
		if(Pmode_T >= 0.5f)
		{
...
void Display_P_Mode()
{
    ...
    if (PMode.Status) // 进入PMode
    {
        if (!Pmode_ShowFlag)
        {
            seg1 = 0x3f;//P
            seg2 = 0x10;//-
            seg3 = DOWNR_Tab[PMode.Option];//显示0
        }
        else
        {
            seg1 = 0x00;//不显示
