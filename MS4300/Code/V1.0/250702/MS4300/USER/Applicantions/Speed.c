...
void Encoder_Init(void)
{
    ...
    HAL_TIM_IC_Start_IT(&htim16, TIM_CHANNEL_1);//开启time3通道1输入捕获
}
...
void CaptureChanneCallback(void)
{
    ...
    frq = 10000.0f / (float)capture;//计算频率
    capture2 = capture1;
    Speed.Rel = frq * 60 / 5;
    Speed.Stop_Cnt = 0;
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM16)
    {
        ...
        {
            capture1 = __HAL_TIM_GET_COMPARE(&htim16, TIM_CHANNEL_1);//获取Tim3通道1的输入捕获
