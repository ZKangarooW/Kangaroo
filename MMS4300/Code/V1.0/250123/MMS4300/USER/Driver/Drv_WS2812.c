...
{
    WS2812_SetColor(0xFF, 0xFF, 0xFF, 0xFF, 0.6f);//全亮
    if(Work_Num[0].Run_Status != 1 && Work_Num[1].Run_Status != 1 
       && Work_Num[2].Run_Status != 1 && Work_Num[3].Run_Status != 1
        )//系统没启动的话
    {
        WS2812_SetColor(0x0C, 0x00, 0x00, 0x00, 0);//Start
    }
    else
    {
        WS2812_SetColor(0x0C, 0xFF, 0xFF, 0xFF, 0.6f);//Start
    }
    
    if (Lock_Switch)
    {
        WS2812_SetColor(0x0D, 0xFF, 0xFF, 0xFF, 0.6f);//锁定按键
    }
    else
    {
        WS2812_SetColor(0x0D, 0x00, 0x00, 0x00, 0.0f);//锁定按键
    }
    
    if(sys.Sync)
    {
        WS2812_SetColor(0x0E, 0xFF, 0xFF, 0xFF, 0.6f);//Sync
    }
    else
    {
        WS2812_SetColor(0x0E, 0x00, 0x00, 0x00, 0.0f);//Sync
    }
