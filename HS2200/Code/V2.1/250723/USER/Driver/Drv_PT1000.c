...
void AFE_Sample_Handler(void)
{
	...
	AD_T1=((float)ADC_Val_Avg[1]*Vref_3V3/4096)/OP_Value+(R_910R/(R_3K+R_910R)*TL431_V);//计算电压值
	AD_T2=((float)ADC_Val_Avg[0]*Vref_3V3/4096)/OP_Value+(R_910R/(R_3K+R_910R)*TL431_V);//计算电压值
	
	R_value1 = (AD_T1*R_3K)/(TL431_V-AD_T1)*1000;//乘1000是因为算出来是千欧为单位的，要转化成欧姆
	R_value2 = (AD_T2*R_3K)/(TL431_V-AD_T2)*1000;//乘1000是因为算出来是千欧为单位的，要转化成欧姆
	
	PT_VALUE_1_TEMP=R_value1 * (1000 /Param.Correct);//计算电阻值
	PT_VALUE_2_TEMP=R_value2 * (1000 /Param.Correct);//计算电阻值
	if(PT_VALUE_2_TEMP > 1758.56)
	{
		PT_VALUE_2_TEMP = 1758.56f + (2156.08f-1758.56f)*((PT_VALUE_2_TEMP-1758.56f)/(1940.98f-1758.56f));
	}
}
...
int AFE_GetTemperature(float value_adc)
{
