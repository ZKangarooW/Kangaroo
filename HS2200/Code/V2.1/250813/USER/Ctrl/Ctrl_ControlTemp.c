...
static void Outside_Ctrl(float dT, int32_t Ctrl_temp)
{
	if(sys.HeatMode_Type == A1)
	{
		...
		else if (Temp.Outside < Ctrl_temp - 50 && Ctrl_temp <= 70)//达到目标温度-10℃前全速加热
		{
				Mesa_Ctrl(dT, Ctrl_temp*1.5f);
		}
		else if (Temp.Outside < Ctrl_temp - 80 && Ctrl_temp > 70)//达到目标温度-10℃前全速加热
		{
				Mesa_Ctrl(dT, Ctrl_temp*2.4f);
		}
		else//PID控制
		{//设定加热周期为1000ms，1000分之PID_Para.pid_result毫秒开加热器
			if(Ctrl_temp>700)
	...
	{
		if (Temp.Outside < Ctrl_temp - 100)//达到目标温度-15℃前全速加热
		{
			Mesa_Ctrl(dT, Ctrl_temp*1.7f);
		}
		else if (Temp.Outside < Ctrl_temp - 80)//达到目标温度-10℃前全速加热
		{
			Mesa_Ctrl(dT, Ctrl_temp*1.5f);
		}
		else if (Temp.Outside < Ctrl_temp - 40)//达到目标温度-10℃前全速加热
		{
			Mesa_Ctrl(dT, Ctrl_temp*1.3f);
		}
		else//PID控制
		{//设定加热周期为1000ms，1000分之PID_Para.pid_result毫秒开加热器
			if(Ctrl_temp>700)
