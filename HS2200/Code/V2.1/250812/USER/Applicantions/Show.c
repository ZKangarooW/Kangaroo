...
uint8_t TimeType_ShowFlag,HeatType_ShowFlag;
...
static void Icn_Twinkle(float dT)
{
	...
	if(sys.Run_Status)		
	{
		...
		if(T >= 0.5f)
		{
			...
			{
				if(Time.Rel && (!Temp.Ctrl || TempIcn_ShowFlag != TimeIcn_ShowFlag) && (Time.Rel != Time.Ctrl) && (Temp.Set == 0 || Temp.ADDMode == 3 ))
...
/*
*****************************************************************
 * 函数原型：static void SettingMode_Twinkle(float dT)
 * 功    能：设置模式闪烁
 * 调    用：内部调用
*****************************************************************
*/
static void SettingMode_Twinkle(float dT)
{
	static float T;
	if(sys.Setting && Key_Status==0)		
	{
		T += dT;
		if(T >= 0.5f)
		{
			if(sys.Setting_Option == 1)
				TimeType_ShowFlag = ~TimeType_ShowFlag;//温度图标闪烁;
            else
                TimeType_ShowFlag = 0;
			
			if(sys.Setting_Option == 2)
				HeatType_ShowFlag = ~HeatType_ShowFlag;//温度图标闪烁;
            else
                HeatType_ShowFlag = 0;
	...
	{
		TimeType_ShowFlag = 0;//不显示速度图标
		HeatType_ShowFlag = 0;//不显示温度图标
	}
}
...
void DownTime_Display(void)
{
    ...
    /*************发送数据***************/	
	Write_Addr_Dat_N(30, 0x00,1);
	Write_Addr_Dat_N(28, 0x00,1);
	Write_Addr_Dat_N(26, 0x00,1);
	...
	Write_Addr_Dat_N(12, seg3,1);
	Write_Addr_Dat_N(14, seg4,1);
