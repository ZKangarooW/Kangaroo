...
/**********局部变量声明******/
uint16_t WS2812_Buf[RGB_NUM * 24+DUMMY_WORD]; // 存放的数组
...
void WS2812_Update(void)
{
	...
	HAL_TIM_PWM_Start_DMA(WS2812.Tim, WS2812.CHANNEL, (uint32_t *)WS2812_Buf, Memaddr+DUMMY_WORD); // 开始传输字节数据
...
void WS2812_SetColor(uint8_t num, uint8_t r, uint8_t g, uint8_t b,float light)
{
	...
	WS2812.Red_Out = (WS2812.Red_Out + 1) / 16 * light * 10;		// 乘等于最大亮度值
	WS2812.Green_Out = (WS2812.Green_Out + 1) / 16 * light * 10; // 乘等于最大亮度值
	WS2812.Blue_Out = (WS2812.Blue_Out + 1) / 16 * light * 10;	// 乘等于最大亮度值
...
void KEYLED_Check(float dT)
{
	if (Lock_Switch) // 按下按键
	{
		WS2812_SetColor(0x00, 0xFF, 0xFF, 0xFF, MAX_RATE);
	}
	else
	{
		WS2812_SetColor(0x00, 0x00, 0x00, 0x00, MAX_RATE);
	}

	if (Shake_Switch) // 按下按键
	{
		WS2812_SetColor(0x01, 0xFF, 0xFF, 0xFF, MAX_RATE);
	}
	else
	{
		WS2812_SetColor(0x01, 0x00, 0x00, 0x00, MAX_RATE);
	}
}
...
void ShakeRGB_Check(float dT)
{
	...
	{
	case 0:
		WS2812_SetColor(0xFF, 0xFF, 0x00, 0x00, MAX_RATE);
		break;
	case 1:
		WS2812_SetColor(0xFF, 0x00, 0x00, 0x00, MAX_RATE);
		break;
	case 2:
		WS2812_SetColor(0xFF, 0xFF, 0x00, 0x00, MAX_RATE);
		break;
...
void WS2812_Duty(float dT)
{
	...
	if (led_check) // 开机灯光自检
	{
		led_check = 0, WS2812_Status.Mode = 1;
	}
	else
	{
		...
		{
		case 1:
			WS2812_SetColor(0xFF, 0xFF, 0xFF, 0xFF, MAX_RATE);
//			KEYLED_Check(dT);
			break; // 正常情况下
		case 2:
			ShakeRGB_Check(dT);
