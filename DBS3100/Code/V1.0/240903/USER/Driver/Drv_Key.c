...
void Key_Scan(float dT)
{
    ...
    if (Key_Flag1) // 按键被按下
    {
        if (Key1 == Key_Up) // 抬起按键
        {
            ...
            {
				if (sys.Run_Status == 1 || Calibration.Switch)
				{
					Beep_Flash = 2;
				}
				else
            ...
            if (Key_Cnt3 < 1.5f) // 按键时间小于1.5S是单击
            {
				...
				else
				{
                    if(Calibration.Switch)
                    {
                        ...
                        if (Calibration.Deviation > 850)
							Calibration.Deviation = 850;
                        Param.Calibration = Calibration.Deviation;//温度校准值
                        Save_Param_En = 1; // 保存
                    }
        ...
        if (Key4 == Key_Up) // ̧抬起按键
        {
            if (Key_Cnt4 < 1.5f) // 按键时间小于1.5S是单击
            {
				...
				else
				{
                    ...
                    {
                         Calibration.Deviation --;
                        if (Calibration.Deviation < 650)
							Calibration.Deviation = 650;
                        Param.Calibration = Calibration.Deviation;//温度校准值
                        Save_Param_En = 1; // 保存
                    }
					else if (sys.SetMode_Option == 1)
    ...
    if (Key_Flag5) // 按键被按下
    {
        if (Key5 == Key_Up) // 抬起按键
        {
            ...
            {
                if ((!HALL1 || !HALL2) && (Temp.Set>Temp.MIN || Speed.Set > 0))
                {
                    if (sys.Run_Status == 1 || Calibration.Switch)
                    {
                        sys.Run_Status = 2;
