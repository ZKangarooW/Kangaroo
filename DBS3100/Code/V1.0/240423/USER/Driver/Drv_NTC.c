...
uint16_t ADC_Val1, ADC_Val2,ADC_Val3;
#define N 50         // 采集N次
int ADCvalue_Buf[N]; // 用于储存采集到的adc值
int i = 0;
int ADCvalue_Buf1[N]; // 用于储存采集到的adc值
int j = 0;
int ADCvalue_Buf2[N]; // 用于储存采集到的adc值
int k = 0;
...
int Filter_ADC2(void)
{
    ...
    ADCvalue_Buf2[k++] = ADC_Val[2]; // 将adc的值储存
...
void Read_Temp(float dT)
{
    ...
    ADC_Val3 = Filter_ADC2();
    
    if (T >= 0.2f) // 1S
    {
        sys.Temp_V = ADC_Val3*(3.3/4096);//先转换成电压
        sys.Temp =  (1.43-sys.Temp_V)/0.0043+25;
//        sys.Temp = (sys.Temp_V-0.76)/0.0025+25;//转换成温度
