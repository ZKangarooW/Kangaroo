...
void Display_Temp(int16_t dis_set_temp, int16_t dis_rel_temp)
{
    ...
    if (!Temp_ShowFlag)
    {
        ...
        if(Param.type)//单加热显示‘----’
        {
             if (dis_set_temp == Temp.MIN ) // 等于-0.1度
             {
                Temp_QD = Tab1[10];
                Temp_BD = Tab1[10];
                Temp_SD = Tab1[10];
...
void Display_Speed(int16_t dis_set_speed, int16_t dis_rel_speed)
{
    ...
    /**********加热图标**********/
    if (!Temp.Icon && (sys.Run_Status == 1) && Icn_Falg && Temp.Set != Temp.MIN)
    {
        seg9 &= 0xFE;seg9 |= 0x01;
    }
    else
    ...
    /**********制冷图标**********/
    if(Param.type == 0 && Temp.Set != Temp.MIN)//加热制冷
    {
	...
	{
        if(sys.Run_Status == 0 && Hall_Flag == 1)
        {
            Speed.MAX = 800;
...
void Show_Display(void)
{
    ...
    if(Calibration.Switch)
    {
        Display_Temp(Calibration.Deviation,Temp.Display_Rel);
    }
    else
    {
